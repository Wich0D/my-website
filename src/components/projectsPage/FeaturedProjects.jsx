...
export default function FeaturedProjects() {
    ...
    // Filter all projects for the selected role
    const allRoleProjects = useMemo(() => {
        return project_list.filter((project) => 
            project.roles?.includes(selectedRole)
        );
    }, [selectedRole]);
    ...
    // Handle image cycling for the expanded project
    useEffect(() => {
        ...
        }, 3000); // Increased slightly for smoother pacing
    ...
    const handleCardClick = (index) => {
        if (expandedIndex === index) {
            ...
            setCurrentImageIndex(0); // Reset image index on new selection
        }
    };
        ...
        <section className="flex flex-col gap-8 items-center w-full px-4 md:px-8 py-4">
            ...
            {featuredProjects.length === 0 ? (
                ...
                <>
                    ...
                    <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-6xl ${hasExpanded ? "lg:grid-cols-2" : "lg:grid-cols-3"}`}>
                        {featuredProjects.map((project, index) => {
                            const isExpanded = expandedIndex === index;
                            const isMinimized = hasExpanded && !isExpanded;
                            ...
                            return (
                                <div 
                                    ...
                                    className={`bg-[#132042] rounded-2xl overflow-hidden flex cursor-pointer transition-all duration-500 ease-in-out shadow-lg group ${
                                        isExpanded 
                                ...
                                >
                                    {/* Image Container */}
                                    <div className={`relative overflow-hidden bg-[#0d152e] transition-all duration-500 flex-shrink-0 ${
                                        isExpanded 
                                        ...
                                        <AnimatePresence>
                                            {imageSrc ? (
                                                ...
                                                <div className="flex items-center justify-center w-full h-full text-gray-500 absolute inset-0">No image</div>
                                            )}
                                        </AnimatePresence>
                                    </div>
                                            ...
                                            <>
                                                ...
                                                <div className="mt-auto">
                                                    ...
                                                    <div className="flex gap-4">
                                                        {project.src_access && project.src[0] && (
                                                            <a href={project.src[0]} target="_blank" rel="noopener noreferrer" 
                                                               onClick={(e) => e.stopPropagation()}
                                                               className="text-gray-400 hover:text-main-cyan transition-colors">
                                                                <FaGithub size={isExpanded ? 26 : 22} />
                                                            </a>
                                                        )}
                                                        {project.src_access && project.src[1] && (
                                                            <a href={project.src[1]} target="_blank" rel="noopener noreferrer" 
                                                               onClick={(e) => e.stopPropagation()}
                                                               className="text-gray-400 hover:text-main-cyan transition-colors">
                                                                <FaExternalLinkAlt size={isExpanded ? 24 : 20} />
                                                            </a>
                                                        )}
                                                    </div>
                                                </div>
                    ...
                    <AnimatePresence>
                        {showMore && remainingProjects.length > 0 && (
                            ...
                            >
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl mx-auto pt-4 pb-8">
                                    {remainingProjects.map((project, index) => (
                                        <div 
                                            key={`remaining-${index}`} 
                                            className="bg-[#132042] rounded-2xl overflow-hidden flex flex-col hover:-translate-y-1 transition-all duration-300 shadow-lg group"
                                        >
                                            <div className="relative w-full h-48 md:h-56 overflow-hidden bg-[#0d152e]">
